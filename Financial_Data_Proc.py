...
## Takes inputs from Santander API to calculate the possible mortgage
# repayment rate and total value of the mortgage you can afford


# Inputs - Pandas data table from Emma Index/Date/Money out (-ve if
# incoming) / Balance / Who / Transaction Type / Account_Number, Savings
# from UI double
...
def Analyse_Account():
    ...
    LastYearTransactions = df[
        df['Date'] >= pd.datetime(MostRecentDate.year - 1,
                                  MostRecentDate.month, MostRecentDate.day)]
    ...
    Incomes = -1 * np.sum(
        np.array(LastYearTransactions['Amount'], dtype='double'))

    # If the time of the transactions is less than one year, make adjstment
    # to annual savings
    ...
    start_date = dt.datetime(MostRecentDate.year - 1, MostRecentDate.month,
                             MostRecentDate.day)
    Year = start_date.year
    for month in range(1, 13):
        Last_Day_In_Month = dt.date(Year, np.mod(start_date.month + month - 1,
                                                 12) + 1, 1) - dt.timedelta(
            days=1)
        if np.mod(start_date.month + month - 1, 12) == 11:
            Year = Year + 1
        First_Day_In_Month = dt.date(Last_Day_In_Month.year,
                                     Last_Day_In_Month.month, 1)
        Temp1 = df[df['Date'] >= First_Day_In_Month]
        Temp2 = Temp1[Temp1['Date'] <= Last_Day_In_Month]['Amount']
        try:
            Salaries[month - 1] = np.min(np.array(Temp2,
                                                  dtype='double'))  # Use
            # min as income is negative
        except ValueError:
    ...
    for month in range(1, 13):
        ...
        try:
            Rents[month - 1] = np.max((np.array(Temp2,
                                                dtype='double')))  # Use min
            #  as income is negative
        except ValueError:
